...
import { ChartEditStorage } from '@/store/chartEditStore/chartEditStore.d';
import { PreviewScaleEnum } from '@/enums/styleEnum';

export const useScale = (localStorageInfo: ChartEditStorage) => {
  const entityRef = ref();
  ...
  onMounted(() => {
    switch (localStorageInfo.editCanvasConfig.previewScaleType) {
      ...
      case PreviewScaleEnum.SCROLL_Y:
        (() => {
          const { calcRate, windowResize, unWindowResize } =
            usePreviewScrollYScale(
              width.value as number,
              height.value as number,
              previewRef.value,
