...
} from '@/store/chartHistoryStore/chartHistoryStore.d';
...
export const useChartEditStore = defineStore({
  ...
  actions: {
    ...
    fetchTargetIndex(id?: string): number {
      ...
      const targetIndex = this.componentList.findIndex(
        (e) => e.id === targetId
      );
    ...
    setCopy(isCut = false) {
      try {
        ...
        if (this.getTargetChart.selectId.length > 1) return;
        // 处理弹窗普通复制的场景
        if (document.getElementsByClassName('n-modal-body-wrapper').length)
          return;
    ...
    setParse() {
      try {
        ...
        this.addComponentList(
          parseHandle(recordCharts.charts),
          undefined,
          true
        );
        if (isCut) {
      ...
      if (isGroup || isUnGroup) {
        if ((isGroup && isForward) || (isUnGroup && !isForward)) {
          ...
          if (historyData.length > 1) {
            ...
            (historyData[0] as CreateComponentGroupType).groupList.forEach(
              (item) => {
                ids.push(item.id);
              }
            );
