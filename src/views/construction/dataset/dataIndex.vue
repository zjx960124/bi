  ...
  <div class="excel-create-container">
        ...
        <el-dropdown>
          ...
          <template #dropdown>
            <el-dropdown-menu class="left-dropdown">
              <el-dropdown-item @click.stop="openDialog(3)">Excel数据集</el-dropdown-item>
              <el-dropdown-item @click.stop="openDialog(2)">SQL数据集</el-dropdown-item>
              <el-dropdown-item @click.stop="openDialog(1)">数据库数据集</el-dropdown-item>
              <el-dropdown-item divided @click.stop="openDialog(0)">新建文件夹</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
    ...
    <div class="right">
      ...
      <div class="right-content">
        <el-empty :image="imgUrl" v-if="dataList && dataList.length == 0" class="datasource-empty"
          description="请先上传Excel数据" />
        <div class="tableBox el-common-tableBox">
          <el-table :data="tableData" align="center" fit :highlight-current-row="false" :border="false"
    ...
    <el-dialog v-model="dialogTableVisible" :width="500" title="新建文件夹" @close="">
    ...
    <el-dialog v-model="addNewDataSetDialog" :width="500" title="新建数据集">
      <div class="form">
        <el-form ref="ruleFormRef" :model="ruleForm" :rules="rules" label-width="120px" class="demo-ruleForm"
          size="small" status-icon>
          <el-form-item label="所属文件夹" required>
            <el-cascader v-model="fileId" :options="options" :props="props1" clearable />
          </el-form-item>
          <el-form-item label="数据类型" prop="dataSetType">
            <el-select v-model="dataSetType" class="m-2" placeholder="请选择" clearable>
...
//接口地址
import { addDataSetOrFileFolder, getFileFolderAndDataSets } from '@/api/dataset'
...
//新增数据集
const saveDataSet = () => {
  let ids = fileId?.value[0];
  let dataSetTypes = dataSetType.value;
  if (!ids || !dataSetTypes) return ElMessage.error('所属文件夹或数据类型不能为空');
...
const handleAllFileFolder = (data: any) => {
  ...
  if (arr && arr.length > 0) {
    arr.map((item: any) => {
      if (item.isFileFolder != 2 && item.dataSetDtos && item.dataSetDtos.length > 0) {
        let arrs = [] as Array<any>;
        item.dataSetDtos.map((it: any) => {
          if (it.isFileFolder != 2 && it.dataSetDtos && it.dataSetDtos.length > 0) {
