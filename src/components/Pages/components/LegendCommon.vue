...
<template>
  ...
  <div class="common-item">
    ...
    <n-button @click="legendPosition('top')" round size="tiny"
      :class="{ 'common-active': legendCurrentPosition === 'top' }" type="tertiary" :disabled="!legend.show">
      <n-icon :component="CaretUp"></n-icon>
    </n-button>
    <n-button @click="legendPosition('bottom')" round size="tiny"
      :class="{ 'common-active': legendCurrentPosition === 'bottom' }" type="tertiary" :disabled="!legend.show">
      <n-icon :component="CaretDown"></n-icon>
    </n-button>
    <n-button @click="legendPosition('left')" round size="tiny"
      :class="{ 'common-active': legendCurrentPosition === 'left' }" type="tertiary" :disabled="!legend.show">
      <n-icon :component="CaretBack"></n-icon>
    </n-button>
    <n-button @click="legendPosition('right')" round size="tiny"
      :class="{ 'common-active': legendCurrentPosition === 'right' }" type="tertiary" :disabled="!legend.show">
      <n-icon :component="CaretForward"></n-icon>
    </n-button>
  </div>
  <div class="common-item">
    <div class="common-sub-title">文本</div>
    <el-input-number v-model="legend.textStyle.fontSize" class="common-number-input" :min="1" :max="44"
      controls-position="right" size="small" :disabled="!legend.show" />
    <n-color-picker class="common-color-picker" style="display: inline-block" v-model:value="legend.textStyle.color"
...
" :class="{ commonActive: legend.textStyle.fontWeight === 'bold' }">
    ...
    <div class="commmon-switch-self" :class="{ commonActive: legend.textStyle.fontStyle === 'oblique' }" @click="
  switchCommon(
    legend.textStyle,
    'fontStyle',
